...
export default {
	...
	methods:{
		...
		handleAddUploadSuccess(res, file, fileList){
			this.uploadNum += 1;
			console.log("上传成功呢",res,file,this.uploadNum);
			var url = this.upload_qiniu_addr+res.key;
			this.fileList.push({name: Date.now()+this.getExtName(url),url:url});
		...
		handleExceed(files, fileList) {
			this.$message.warning(`当前限制选择 ${this.imageLimit} 个图片，本次选择了 ${files.length} 个图片，共选择了 ${files.length + fileList.length} 个文件`);
		},		
	}
}
</script>
