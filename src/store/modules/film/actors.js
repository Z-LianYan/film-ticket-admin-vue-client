...
const actions = {
  ...
  add({ commit, state }, requestParams) {
    return new Promise((resolve, reject) => {
      requstTools.post(aipUrl.ADD_ACTORS, requestParams, '正在提交...').then(res => {
        if (res.error == 0) {
  ...
  update({ commit, state }, requestParams) {
    return new Promise((resolve, reject) => {
      requstTools.post(aipUrl.EDIT_ACTORS, requestParams, '正在提交...').then(res => {
        if (res.error == 0) {
          Message.success(res.message);
          resolve();
        } else {
  ...
  del({ commit, state }, requestParams) {
    return new Promise((resolve, reject) => {
      requstTools.post(aipUrl.DEL_ACTORS, requestParams, '正在删除...').then(res => {
        if (res.error == 0) {
          Message.success(res.message);
          resolve();
        } else {
