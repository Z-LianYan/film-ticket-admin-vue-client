    ...
    <el-card class="box-card">
      ...
      >
        <el-form-item label="申请类型" prop="type">
          <el-select v-model="ruleForm.type" placeholder="请选择所属角色" v-if="auditType.length">
            <el-option
        ...
        <div style="position:relative;display: inline-block;">
          <el-form-item label="开始时间" prop="start_leave_date">
            <el-date-picker
              v-model="ruleForm.start_leave_date"
              @change="onStartDateChange"
              type="date"
              placeholder="选择开始时间"
            ></el-date-picker>
            <el-select v-model="ruleForm.start_leave_type" placeholder="">
              <el-option label="上午" :value="1"/>
              <el-option label="下午" :value="2"/>
            </el-select>
          </el-form-item>
          <el-form-item label="结束时间" prop="end_leave_date">
            <el-date-picker
              v-model="ruleForm.end_leave_date"
              @change="onEndDateChange"
              type="date"
              placeholder="选择结束时间"
            ></el-date-picker>
            <el-select v-model="ruleForm.end_leave_type" placeholder="">
              <el-option label="上午" :value="1"/>
              <el-option label="下午" :value="2"/>
            </el-select>
          </el-form-item>
        ...
        <el-form-item label="请假类型" prop="leave_type">
          <el-select v-model="ruleForm.leave_type" placeholder="请选择所属角色">
            <el-option
        ...
        <el-form-item label="请假原因" prop="leave_reason">
          <el-input v-model="ruleForm.leave_reason" placeholder="请输入请假原因" type="textarea" :rows="4"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        admin_id: [
          { required: true, message: "请选择请假人", trigger: ["blur","change"] },
        ],
        start_leave_date: [
  ...
  methods: {
    onStartDateChange(date){
      const { start_leave_date,end_leave_date } = this.ruleForm;
      if(start_leave_date>end_leave_date) {
        this.$message.warning('开始时间不能大于结束时间');
        this.ruleForm.start_leave_date = '';
        return;
      };
      this.ruleForm.start_leave_date = date;
    },
    onEndDateChange(){
      const { start_leave_date,end_leave_date } = this.ruleForm;
      if(start_leave_date>end_leave_date) {
        this.$message.warning('开始时间不能大于结束时间');
        this.ruleForm.end_leave_date = '';
        return;
      };
      this.ruleForm.end_leave_date = date;
    },
    calcLeaveDay(){
      const { start_leave_date,end_leave_date, start_leave_type, end_leave_type } = this.ruleForm;
